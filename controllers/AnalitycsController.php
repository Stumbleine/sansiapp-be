...
class AnalitycsController extends \yii\web\Controller
{
  ...
  public function getVisits($rol = "PRV", Empresa $empresa = null)
  {
    $connection = \Yii::$app->getDb();
    if ($rol === 'PRV') {
      $query = $connection->createCommand("select b.id_beneficio, b.titulo,b.image, b.status,e.rubro, count(*) 
        from  visualizacion v, beneficio b
        where v.id_beneficio  = b.id_beneficio and b.id_empresa = " . $empresa->id_empresa . " 
        group by b.id_beneficio, b.titulo,b.status,b.image, e.rubro having count(*)>=1 order by count(*) desc");
    } else if ($rol === 'ADM') {
      $query = $connection->createCommand("select v.id_beneficio, b.titulo ,b.status, b.image,e.razon_social,e.rubro, count(*) 
        ...
        group by v.id_beneficio, b.titulo,b.status,b.image ,e.razon_social, e.rubro having  count(*)>=1 order by count(*) desc");
    }
    return $query->queryAll();
  }
  ...
  public function getRedeemed(string $rol = "PRV", Empresa $empresa = null)
  {
    ...
    if ($rol === 'PRV') {
      $query = $connection->createCommand("select c.id_beneficio ,e.razon_social, count(*) from codigo c ,beneficio b,empresa e  
        ...
        group by c.id_beneficio ,e.razon_social having  count(*)>=1 order by count(*) desc");
    } else if ($rol === 'ADM') {
      $query = $connection->createCommand("select c.id_beneficio ,e.razon_social, count(*) from codigo c ,beneficio b,empresa e  
